...
class AMCN_S3_Uploads extends AMCN_S3_Uploads_Public {

    function AMCN_S3_Uploads() {
		parent::AMCN_S3_Uploads_Public();
		if (!file_exists(dirname(__FILE__).'/config.php')) {
			//We don't need this for now. Every config option is stored in s3-config.php - add_action('admin_menu', array(&$this, 'settings'));
		}

        if (isset($this->options['enable-s3-tab']) && $this->options['enable-s3-tab']) {
			...
			add_action('media_upload_amcn-s3-uploads', array(&$this, 'media_upload_content'));
		}
        add_action('activate_amcn_s3/wordpress-s3.php', array(&$this, 'activate'));
        if (isset($_GET['amcnS3Activate']) && $_GET['amcnS3Activate'] == 'wordpress-s3') {
    ...
    function activate() {
        wp_redirect('plugins.php?amcnS3Activate=wordpress-s3');
        exit;
    }
    function deactivate() {}

    function showConfigNotice() {
        add_action('admin_notices', create_function('', 'echo \'<div id="message" class="updated fade"><p>Amazon S3 Plugin for WordPress <strong>activated</strong>. <a href="options-general.php?page=amcn-s3-uploads/inc/class-plugin.php">Configure the plugin &gt;</a></p></div>\';'));
    }
    ...
    function addhooks() {
		parent::addhooks();
        if(isset($this->options['enable-uploads']) && $this->options['enable-uploads']){
    ...
    function version_check() {
        ...
        if (is_object($amcn_s3_version_check)) {
            ...
            $amcn_s3_version_check->versionCheck(668, $data['Version']);
        }
    }
    function admin() {
        if ( isset( $_POST['action'] ) && $_POST['action'] == 'save' ) {
            if (!is_array($_POST['options'])) $_POST['options'] = array();
            $options = get_option('amcn_s3_uploads');
	...
	*/
	function delete_attachment( $post_id ) {
        ...
        if (!$this->options['enable-plugin'] || !$this->options['bucket'] || !$this->options['secret']) {
            return;
        }
		...
		require_once(dirname(__FILE__).'/lib.s3.php');
		$this->s3 = new AMCN_S3($this->options['key'], $this->options['secret']);
    ...
    */
    function wp_update_attachment_metadata($data, $postID) {
        ...
        $site_prefix = isset($this->options['site-bucket']) && $this->options['site-bucket'] ? $this->options['site-bucket'].'/' : '';
        $prefix = $site_prefix.substr($parts['path'], 1) .'/';
	...
	function media_buttons($context) {
		...
		$out = ' <a href="'.$media_upload_iframe_src.'&tab=amcn-s3-uploads&TB_iframe=true&height=500&width=640" class="thickbox" title="'.$image_title.'"><img src="'.$image_btn.'" alt="'.$image_title.'" /></a>';
		return $context.$out;
	}
	function media_upload_tabs( $tabs ) {
		$tabs['amcn-s3-uploads'] = 'Amazon S3';
		return $tabs;
	}
	function media_upload_content() {
        $this->upload_files_amazon_s3(); // process any uploaded files or new folders
    ...
    */
    function addPhotosTab() {
        ...
        add_action('upload_files_amazon_s3', array(&$this, 'upload_files_amazon_s3'));
	...
	*/
        ...
        if ($this->options['key'] && $this->options['secret'] && $this->options['bucket']) {
            ...
            $tab = array(
                'amcn_amazon_s3' => array('Amazon S3', 'upload_files', array(&$this, 'tab'), $paged, $args),
                //'amcn_amazon_s3_upload' => array('Upload S3', 'upload_files', array(&$this, 'upload'), $paged, $args),
                );
    ...
    function upload_files_amazon_s3() {
		global $current_blog;
